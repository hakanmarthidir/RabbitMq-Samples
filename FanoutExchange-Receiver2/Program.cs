...
namespace FanoutExchange_Receiver2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (var connection = factory.CreateConnection())
            {
                using (var channel = connection.CreateModel())
                {
                    ...
                    //NewYorkTimes kuyrugundan gerekli haberleri alıp istedigini yapabilir. Aynı mesajı farklı işlemler için kullanabiliriz. 
                    channel.BasicConsume(queue: "newsfanout_newyorktimes", autoAck: false, consumer: consumer);
                    Console.ReadLine();
                }
            }
        }
